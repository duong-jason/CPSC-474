...
For example: The $Occupation$ feature, with $Lawyer$, $Teacher$, and $Doctor$ as the possible levels, has the highest information gain and is chosen from dataset $\mathcal{D}$ for which to split. A branch is grown for each level with partitioned dataset $\mathcal{D}[Occupation=d]\ \text{for all}\ d\in\\lbrace Lawyer, Teacher, Doctor\rbrace$ containing all instances for which $Occupation=d$.
